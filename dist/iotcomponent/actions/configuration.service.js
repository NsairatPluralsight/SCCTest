...
const iot_component_type_1 = require("../models/iot-component-type");
class ConfigurationService {
    ...
    */
    ...
    /**
      ...
      * @return {Promise<Result>} Result enum wrapped in a promise.
      */
    async getAllIoTTypes(message) {
        try {
            let deviceRepo = new type_repository_1.IoTTypeRepository();
            let data = await deviceRepo.getAll(new iot_component_type_1.IoTComponentType());
            let payload = new response_payload_1.ResponsePayload();
            if (data) {
                payload.data = data;
                payload.result = enum_1.Result.Success;
    ...
    */
    async getIoTType(message) {
        try {
            ...
            let deviceRepo = new type_repository_1.IoTTypeRepository();
            let data = await deviceRepo.get(new iot_component_type_1.IoTComponentType(), params);
            let payload = new response_payload_1.ResponsePayload();
            if (data) {
