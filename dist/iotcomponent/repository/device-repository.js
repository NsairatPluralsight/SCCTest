...
class DeviceRepository {
    ...
    */
    async getAll(entity) {
        try {
            let tableName = await database_helper_1.DatabaseHelper.getTableName(entity);
    ...
    * @param {IoTComponent} entity - entity
    * @param {Array<KeyValue>} params - the paramters for the sql query
    * @returns {Promise<any>} - IoT component column configuration as JSON string.
    */
    async getColumn(params, columnName) {
        try {
            let paramsArray = new Array();
            let sqlCondition = new key_value_1.KeyValue('sqlCondition', '');
            await database_helper_1.DatabaseHelper.prepareCondition(params, paramsArray, sqlCondition);
            ...
            if (columnData && columnData[0]) {
                return columnData[0][database_helper_1.DatabaseHelper.jsonKey];
            }
            else {
                return null;
            }
